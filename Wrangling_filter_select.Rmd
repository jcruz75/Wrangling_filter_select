...
The amount of movies that were released in 1995 is 35.
...
There are zero comedies with an MPAA rating of over 8.0 and is also 120 minutes. 

```{r}
movies_1995 <- movies %>%    
  filter(year >= 1995 & Action == 1) %>%
  drop_na(budget) %>%
  select(Comedy, title, rating, length,)
...
There are only three movies included in the list that are harry potter movies and these are Harry Potter and the chamber of secrets, Harry potter and the prisoner of azkaban, and harry potter and the sorcerer's stone. The highest rates is Harry potter and the prisoner of Azkaban. The one with the most votes is Harry Potter and the Sorcerer's stonne. 
...
The highest budget for a movie that is either an animation or comedy is with at least 100 votes is 95000000.

```{r}
movies_1995 <- movies %>%    
  filter(year >= 1995 & Action == 1) %>%
  drop_na(budget) %>%
  select(Comedy,Animation, title, votes, budget)
